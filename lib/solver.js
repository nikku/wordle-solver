...
export function scoreLetters(history, words) {
  ...
  const lettersByColumn = getColumnLetterDistribution(
    remainingWords.map(
  ...
  const unknownLettersDistribution = remainingLetters.filter(
    ...
    (lettersDistribution, letter) => {
      ...
      const letterDistribution = lettersByColumn.reduce((score, columnDistribution) => {
  ...
  lettersByColumn.forEach((letterDistribution, idx) => {
  ...
  matched.forEach(letter => {
    lettersByColumn.forEach(letterDistribution => {
