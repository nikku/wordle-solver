...
export function scoreLetters(history, words) {
  ...
  const remainingSlots = matched.reduce((count, m) => count + (m ? 0 : 1), 0);

  const q = remainingLetters.length / remainingSlots;

  // if guessing is our best chance, do it
  if (remainingWords.length < 3 || q < 3 || history.length === 5) {
  ...
  const remainingWordsUnmatched = remainingWords.map(
    word => Array.from(word).map(
      (letter, idx) => matched[idx] || containedLetters.includes(letter) ? '_' : letter
    ).join('')
  );
  ...
  containedLetters.forEach(letter => {
    lettersByWord[letter || '_'] = 0;
  });

  matched.forEach(letter => {
    lettersByWord[letter || '_'] = -.00001;
  });
